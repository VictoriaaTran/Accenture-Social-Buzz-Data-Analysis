...
# ids = set(df['Content ID'].tolist())
# print(len(ids))
# ids = set(df2['Content ID'].tolist())
# print(len(ids))
# save as new csv file
# df.to_csv('ContentOutput.csv')
# df2.to_csv('ReactionsOutput.csv')
# df3.to_csv('ReactionTypesOutput.csv')
...
# df2_1 = pd.merge(df2, df, on="Content ID",how='left')
# # df2_1.dropna(inplace=True)
# print('df2_1', df2_1)
# rows_with_nan = df2_1[df2_1.isnull().any(axis=1)]
# print('nan row:',rows_with_nan)
...
for index, row in reaction_merge.iterrows():
  ...
  #counting the reactions by month
  if month in month_count:
    month_count[month] += 1
  else:
    month_count[month] = 1

#sorted the count of each month from most to least
sorted_month_count = dict(sorted(month_count.items(), key=lambda x: x[1], reverse=True))
print(sorted_month_count)
print("Monthly post with most reactions")
for index, count in sorted_month_count.items():
  print(f'{index}: {count}')

#sorted the count by month
print('sorted by month')
sorted_by_month = dict(sorted(month_count.items(), key= lambda x: x[0]))
for index, count in sorted_by_month.items():
  print(f'{index}: {count}')
...
for x, y in sorted_by_month.items():
  monthX.append(x)
  countY.append(y)

#create a line chart
plt.figure(figsize=(8,6))
plt.plot(monthX, countY, marker = 'o', linestyle='-')

#add labels
plt.title('Posting Counts over Months')
plt.xlabel('Month')
...
#adding value at each data point
for month,count in zip(monthX,countY):
  plt.text(month,count, f'{count}', ha='center', va='bottom')
#display plot
plt.show()
